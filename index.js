...
class MusicVisualizer {
  ...
  init(mountElement) {
    ...
    this.mount.addEventListener('wheel', this.onWheel.bind(this), { passive: false })
  ...
  addLine(fftValues) {
    ...
    const base = 0xe1e1e1
    const color = Math.min(0xffffff, base + (jitter << 16) + (jitter << 8) + jitter)
  ...
  moveLines() {
    ...
    this.lines.children.forEach((plane) => {
      for (let i = 0; i < 400; i++) {
        ...
        if (i < 200) {
          plane.children[0].geometry.attributes.position.array[i * 3 + 2] -= speed
        }
      }
  ...
  requestLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        this.updateLocationHud(`Lat: ${latitude.toFixed(4)}, Lon: ${longitude.toFixed(4)} — resolving…`)
